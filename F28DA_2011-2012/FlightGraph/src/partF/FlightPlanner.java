...
public class FlightPlanner {
	...
	private static void calculateRoutes() {
		...
		System.out.format("%3s%15s%7s%7s%15s%7s%7s", "\nLeg", "Leave", "At", "On", "Arrive", "At", "Cost\n");
		for(Flight f : flightList) {
			...
			totalPeriod = totalPeriod.plus(flightPeriod);
			System.out.format("%3s%15s%7s%7s%15s%7s%7s", fCount, f.getOrigin(), departure.toString("HH:mm"), f.getCode(), f.getDestination(), arrival.toString("HH:mm"), Double.toString(f.getWeight())+"\n");
		}
		/*  Display total cost and totoal period of time in the air. 
		...
		restart(new String[]{
			...
			"Total Time in the Air: "+hoursMinutes.print( totalPeriod.normalizedStandard()),
			"",
