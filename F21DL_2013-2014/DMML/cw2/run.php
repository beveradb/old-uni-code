...
if (!isset($_GET['action'])) {
	header("Location: http://andrewbeveridge.co.uk/coursework/dmml/cw2/run.php?action=report");
	die();
}
...
if(isset($_GET['fieldsToDrop'])) {
	$dataColsToDrop = str_getcsv(trim($_GET['fieldsToDrop']));
}
if(isset($_GET['dropRangeStart']) && isset($_GET['dropRangeEnd'])) {
	$dataColsToDrop = array();
	for($i=trim($_GET['dropRangeStart']); $i <=trim($_GET['dropRangeEnd']); $i++) {
		$dataColsToDrop[$i] = $i;
	}
	if(isset($_GET['undropFields'])) {
		$undropFields = str_getcsv(trim($_GET['undropFields']));
		foreach($undropFields as $fieldNum) {
			if(isset($dataColsToDrop[$fieldNum])) unset($dataColsToDrop[$fieldNum]);
		}
	}
}
...
foreach($dataArrayNoMissingValues as $rowKey => $row) {
	if($file=='yeast') {
		...
		if($row[count($row)-1]==1) $row[count($row)-1]=0.9;
		
		$row[count($row)-1] = substr( $row[count($row)-1]*10, 0, 1 );
	}
...
fisherYatesShuffle($dataArray, $_GET['shuffle']=='r'?rand(0,100):$_GET['shuffle']) ;

$correlationData = array_slice($dataArray,0,floor( count($dataArray)*0.8 ));
$columnCorrelations = array();

for($colKey=0; $colKey<=98; $colKey++) {
	$columnCorrelations[$colKey] = stats_stat_correlation( array_column($correlationData, $colKey), array_column($correlationData, 99) );
}
...
uasort($columnCorrelations,'cmp');

$top5correlatedColumns = array_slice($columnCorrelations, 0, $_GET['topcols'], 1);

echo "<pre>";
echo "Top {$_GET['topcols']} correlated field IDs: ".implode(', ',array_keys($top5correlatedColumns))."\n";
echo "with Pearson's R correlation values: ".implode(', ',$top5correlatedColumns)."\n\n";

foreach($dataArray as $row) {
	foreach($row as $colKey => $colVal) {
		if( array_key_exists($colKey, $top5correlatedColumns) OR $colKey==99 ) $output .= $colVal." ";
	}
	$output .= "\n";
}

file_put_contents("dataArrayTop5CorrelatedColumnsSpaced.txt",$output);
$dataArrayTop5NB = `awk -f /home/andrewbe/public_html/coursework/dmml/cw2/nbFixed.awk < /home/andrewbe/public_html/coursework/dmml/cw2/dataArrayTop5CorrelatedColumnsSpaced.txt`;
//$dataArrayTop5NB = file_get_contents("dataArrayTop5CorrelatedColumnsSpaced.txt");

print_r( substr($dataArrayTop5NB, stripos($dataArrayTop5NB, 'overall accuracy') ) );
...
if(isset($_GET['normalize'])) {
	if($file=="communities") {
		...
		if(count($undropFields)==2) $dataArrayNoMissingValuesSpacedZNorm = `awk -f /home/andrewbe/public_html/coursework/dmml/cw1/znorm3fields.awk < /home/andrewbe/public_html/coursework/dmml/cw1/dataArrayNoMissingValuesSpaced.txt`;
		else $dataArrayNoMissingValuesSpacedZNorm = `awk -f /home/andrewbe/public_html/coursework/dmml/cw1/znorm.awk < /home/andrewbe/public_html/coursework/dmml/cw1/dataArrayNoMissingValuesSpaced.txt`;
		
		file_put_contents("dataArrayNoMissingValuesZNormed.txt",$dataArrayNoMissingValuesSpacedZNorm);
...
if($_GET['action']=='accuracy') {
	...
	if($file=="yeast") {
		...
		$MyData->addPoints($classColumnBucketCounts[0][$_GET['column']],"Class 0");
		$MyData->addPoints($classColumnBucketCounts[1][$_GET['column']],"Class 1");
	}
